...
% subsection costituenti (end)
% section eventi (end)

\section{Previsione e Probabilità} % (fold)
\subsection{Previsione} % (fold)
...
Tramite la coerenza, possiamo dunque definire in modo operazionale la previsione:

\begin{definition}[Primo criterio di coerenza]\label{def:criterio_coerenza_1_previsione}
...
\begin{definition}[Secondo criterio di coerenza]
  Dato un numero aleatorio $X$ soffriamo di una penalità
  \[ P = \left(\frac{X-\bar{x}}{\lambda}\right)^2 \]
  \marginnote{Questa è la definizione secondo De Finetti.
...
\newthought{Le seguenti proprietà} seguono dai criteri di coerenza:
...
% subsection previsione (end)

\subsection{Probabilità di eventi} % (fold)
...
Per una partizione: % DOUBT queste non sono altre proprietà?
...
% subsection probabilita (end)

\subsection{Probabilità e previsione subordinata} % (fold)
...
\begin{definition}[Primo criterio di coerenza]
...
% subsection probabilita_e_previsione_subordinata (end)

\subsection{Probabilità composta} % (fold)
\begin{theorem}[Probabilità composta]\label{thm:probabilita_composta}
  Dati due eventi $A$ e $B$, si ha che:
  \marginnote{In alcuni testi la probabilità composta viene denotata con \( \pr(A\cap B) \).}
...
% subsection probabilita_totale (end)

\subsection{Teorema di Bayes} % (fold)
...
% subsection teorema_bayes (end)

\subsection{Correlazione tra eventi} % (fold)
...
% subsection correlazione_tra_eventi (end)

\subsection{Indipendenza stocastica} % (fold)
...
% subsection indipendenza_stocastica (end)
% section previsione_e_probabilita (end)

\section{Varianza e covarianza} % (fold)
\subsection{Covarianza} % (fold)
...
% section varianza_e_covarianza (end)

\section{Regressione Lineare} % (fold)
\begin{definition}[Regressione lineare]
  ...
  \[ \pr((Y - aX - b)^2) \]
  dove \( Y = aX + b \). % DOUBT e allora a che serve?
...
% section regressione_lineare (end)

\section{Disuguaglianze di Čebyšëv} % (fold)
...
\begin{proof}
  ...
  \[ \pr(X^2) = \pr(X^2|E)\pr(E) + \pr(X^2|\tilde{E})\pr(\tilde{E}) \] % DOUBT manca qualcosa...
...
% section disuguaglianze_di_chebyshev (end)

\section{Legge debole dei grandi numeri} % (fold)
...
\begin{theorem}[Legge debole dei grandi numeri]\label{thm:teorema_bernoulli}
  ...
  % DOUBT varianza o sqm? ed è giusta quella eq? se si che senso ha?
...
% section legge_debole_dei_grandi_numeri (end)

\section{Coefficiente di correlazione} % (fold)
...
% section coefficiente_di_correlazione (end)
